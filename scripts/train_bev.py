...
from skyeye.modules.heads.semantic import BEVSemHead

from skyeye.models.backbone_edet.efficientdet_small import EfficientDet
from skyeye.models.unsupervised_bev import UnsupervisedBevNet, NETWORK_INPUTS_BEV
...
parser = argparse.ArgumentParser(description="SkyEye BEV finetuning script")
...
parser.add_argument("--dataset_root_dir", required=True, type=str, help="The root directory of the data from which the Seam format was generated")
...
parser.add_argument("--bev_percentage", type=float, default=100, help="Percentage of plabels to be loaded")
...
def create_run_directories(args, rank):
    ...
    if args.mode == "train":
        run_dir = os.path.join(experiment_dir, "skyeye_bev_train_{}".format(args.run_name))
    elif args.mode == "test":
        run_dir = os.path.join(experiment_dir, "skyeye_bev_test_{}".format(args.run_name))
    else:
...
def make_dataloader(args, config, rank, world_size):
    ...
    if args.train_dataset == "Kitti360":
        train_db = BEVKitti360Dataset(seam_root_dir=args.seam_root_dir, dataset_root_dir=args.dataset_root_dir,
                                      ...
                                      bev_percentage=args.bev_percentage)
...
def make_model(args, config, fv_num_thing, fv_num_stuff, bev_num_thing, bev_num_stuff):
    ...
    # Create the BEV semantic segmentation network
    bev_sem_loss = SemanticSegLoss(ohem=sem_config.getfloat("bev_ohem"),
                                   class_weights=sem_config.getstruct("bev_class_weights"))
    bev_sem_algo = SemanticSegAlgo(bev_sem_loss, bev_classes['total'], bev_Z_out=Z_out, bev_W_out=W_out)
    bev_sem_head = BEVSemHead(num_classes=bev_classes['total'],
                              bev_W_out=dl_config.getstruct("bev_crop")[0] * dl_config.getfloat("scale"),
                              bev_Z_out=dl_config.getstruct("bev_crop")[1] * dl_config.getfloat("scale"))

    return UnsupervisedBevNet(body, voxel_grid, None, bev_sem_head, voxel_grid_algo, None, bev_sem_algo,
...
def train(model, optimizer, grad_scaler, scheduler, dataloader, meters, **varargs):
    ...
    for it, sample in enumerate(dataloader):
        ...
        sample_cuda = {}
        for key in NETWORK_INPUTS_BEV:
            if isinstance(sample[key], list):
                sample_cuda[key] = [m.cuda(device=varargs['device'], non_blocking=True) for m in sample[key] if not isinstance(m, dict)]
            else:
        ...
        with torch.cuda.amp.autocast():
            losses, results, stats = model(**sample_cuda, do_loss=True, use_fv=False, use_bev=True)
        ...
        bev_sem_conf_stat = stats['bev_sem_conf']

        # Gather the stats from all the workers
        if not varargs['debug']:
            distributed.all_reduce(bev_sem_conf_stat, distributed.ReduceOp.SUM)
        ...
        with torch.no_grad():
            ...
            for result_name, result_value in results.items():
                if result_name.startswith('wt'):
                    meters[result_name].update(result_value.cpu())
            meters['bev_sem_conf'].update(bev_sem_conf_stat.cpu())
...
def validate(model, dataloader, **varargs):
    ...
    bev_num_classes = dataloader.dataset.bev_num_stuff + dataloader.dataset.bev_num_thing
    loss_weights = varargs['loss_weights']

    val_meters = {
        "loss": AverageMeter(()),
        "bev_sem_loss": AverageMeter(()),
        "bev_sem_conf": ConfusionMatrixMeter(bev_num_classes)
    }
    ...
    # Validation metrics
    val_metrics = {"bev_sem_miou": AverageMeter(())}

    # Accumulators for AP, mIoU and panoptic computation
    bev_semantic_conf_mat = torch.zeros(bev_num_classes, bev_num_classes, dtype=torch.double)
    ...
    for it, sample in enumerate(dataloader):
        ...
        with torch.no_grad():
            ...
            if "bev_sem_conf" in stats.keys():
                bev_sem_conf_stat = stats['bev_sem_conf']
            else:
                bev_sem_conf_stat = torch.zeros(bev_num_classes, bev_num_classes, dtype=torch.double)
            rem_stats = {k: v for k, v in stats.items() if (not k.endswith("sem_conf"))}
            if not varargs['debug']:
                distributed.all_reduce(bev_sem_conf_stat, distributed.ReduceOp.SUM)

            # Add the semantic confusion matrix to the existing one
            bev_semantic_conf_mat += bev_sem_conf_stat.cpu()
            ...
            with torch.no_grad():
                ...
                val_meters['bev_sem_conf'].update(bev_sem_conf_stat.cpu())
    ...
    # Finalise mIoU computation
    bev_semantic_conf_mat = bev_semantic_conf_mat.to(device=varargs['device'])
    if not varargs['debug']:
    ...
    bev_sem_union = ((bev_semantic_conf_mat.sum(dim=1) + bev_semantic_conf_mat.sum(dim=0) - bev_semantic_conf_mat.diag()) + 1e-8)
    bev_sem_miou = bev_sem_intersection / bev_sem_union

    # Plot the confusion matrix
    bev_sem_conf_mat_plt = plot_confusion_matrix(bev_semantic_conf_mat, bev_num_classes,
                                                 dataset=dataloader.dataset.dataset_name, bev=True)
    wandb_vis_dict['wandb_bev_sem_conf'] = wandb.Image(bev_sem_conf_mat_plt)

    # Finalize panoptic computation
    # Save the metrics
    scores = {}
    scores['bev_sem_miou'] = bev_sem_miou.mean()
    ...
    del bev_sem_conf_mat_plt, wandb_vis_dict
    ...
    bev_class_names = [name for idx, name in enumerate(dataloader.dataset.bev_categories)]
    logging.log_miou("BEV Sem mIoU", bev_sem_miou, bev_class_names)

    return scores['bev_sem_miou'].item()
...
def main(args):
    ...
    if not args.debug:
        if rank == 0:
            if args.use_wandb:
                wandb_summary = wandb.init(project="skyeye", dir=run_dir,
                                           name="skyeye_bev_train_{}".format(args.run_name), job_type=args.mode,
                                           notes=args.comment,
                                           config=config_dict)
            else:
                wandb_summary = None
    ...
    if args.resume:
        ...
        snapshot = resume_from_snapshot(model, args.resume, ["body", "voxel_grid", "bev_sem_head"])
    elif args.pre_train:
        ...
        pre_train_from_snapshots(model, args.pre_train, ["body", "voxel_grid", "fv_sem_head", "bev_sem_head"], rank)
    ...
    train_meters = {
        ...
        "bev_sem_loss": AverageMeter((), momentum),
        "bev_sem_conf": ConfusionMatrixMeter(train_dataloader.dataset.bev_num_categories, momentum),
    }
    ...
    for epoch in range(starting_epoch, total_epochs):
        ...
        if not args.debug and rank == 0:
            ...
            save_snapshot(snapshot_file, config, epoch, 0, best_score, global_step,
                          ...
                          bev_sem_head=model.module.bev_sem_head.state_dict(),
